...
import CropCatalogTab from "./components/CropCatalogTab";
...
// Remove mock notifications, we will use real ones
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (overviewData?.activities) {
      const stored = localStorage.getItem('readAdminActivities');
      if (stored) {
        try {
  ...
  const handleMarkAllRead = () => {
    if (!overviewData?.activities) return;
    const allIds = overviewData.activities.map((a: any) => a.id);
    const newReadIds = Array.from(new Set([...readActivityIds, ...allIds]));
    setReadActivityIds(newReadIds);
    localStorage.setItem('readAdminActivities', JSON.stringify(newReadIds));
  };

  const getNotificationProps = (type: string, id: string) => {
    const isUnread = !readActivityIds.includes(id);
    switch (type) {
      case 'USER_REGISTERED':
        return { tabType: 'Activities', icon: UserIcon, colorClass: "text-green-600", bgClass: "bg-green-50", unread: isUnread };
      case 'COMPLAINT_RAISED':
        return { tabType: 'Alerts', icon: AlertTriangle, colorClass: "text-amber-600", bgClass: "bg-amber-50", unread: isUnread };
      case 'ORDER_PLACED':
        return { tabType: 'Transactions', icon: ShoppingBag, colorClass: "text-blue-600", bgClass: "bg-blue-50", unread: isUnread };
      case 'CASH_DEPOSIT_PENDING':
        return { tabType: 'System', icon: CreditCard, colorClass: "text-purple-600", bgClass: "bg-purple-50", unread: isUnread };
      case 'PAYOUT_SUCCESSFUL':
        return { tabType: 'System', icon: ShieldCheck, colorClass: "text-green-600", bgClass: "bg-green-50", unread: isUnread };
      default:
        return { tabType: 'System', icon: Info, colorClass: "text-gray-600", bgClass: "bg-gray-50", unread: isUnread };
    }
  };
  ...
  const popoverActivities = (overviewData?.activities || []).map((act: any) => ({
    ...act,
    ...getNotificationProps(act.type, act.id),
    time: getRelativeTime(act.timestamp)
  }));

  const tabCounts = {
    All: popoverActivities.length,
    Alerts: popoverActivities.filter((a: any) => a.tabType === 'Alerts').length,
    Activities: popoverActivities.filter((a: any) => a.tabType === 'Activities').length,
    Transactions: popoverActivities.filter((a: any) => a.tabType === 'Transactions').length,
    System: popoverActivities.filter((a: any) => a.tabType === 'System').length,
  };

  const totalOrders6Months = overviewData?.marketGrowth
    ? overviewData.marketGrowth.reduce((acc: number, curr: any) => acc + curr.orders, 0)
    : 0;

  const totalRevenue6Months = overviewData?.marketGrowth
    ? overviewData.marketGrowth.reduce((acc: number, curr: any) => acc + curr.revenue, 0)
    : 0;

  const newUsers6Months = overviewData?.marketGrowth
    ? overviewData.marketGrowth.reduce((acc: number, curr: any) => acc + curr.users, 0)
    : 0;

  const getGrowthPercentage = (key: 'orders' | 'revenue' | 'users') => {
    if (!overviewData?.marketGrowth || overviewData.marketGrowth.length < 2) return { val: 0, isUp: true };
    const current = overviewData.marketGrowth[overviewData.marketGrowth.length - 1][key];
    const previous = overviewData.marketGrowth[overviewData.marketGrowth.length - 2][key];
    if (previous === 0) {
      return { val: current > 0 ? 100 : 0, isUp: true };
    }
    const diff = current - previous;
    const pct = (diff / previous) * 100;
    return { val: Math.abs(parseFloat(pct.toFixed(1))), isUp: pct >= 0 };
  };

  const ordersGrowth = getGrowthPercentage('orders');
  const revenueGrowth = getGrowthPercentage('revenue');
  const usersGrowth = getGrowthPercentage('users');

  // --- SVG Line Chart Generator for Market Growth ---
  const renderMarketGrowthChart = () => {
    if (!overviewData || !overviewData.marketGrowth) return null;
    const months = overviewData.marketGrowth.map((d: any) => d.month);
    const ordersData = overviewData.marketGrowth.map((d: any) => d.orders);
    const revenueData = overviewData.marketGrowth.map((d: any) => d.revenue);
    const usersData = overviewData.marketGrowth.map((d: any) => d.users);
    ...
    <div className="min-h-screen bg-[#FAFBFA] flex font-[family-name:var(--font-poppins)] text-[#212121]">
      ...
      <aside className="w-64 bg-white border-r border-gray-100 flex flex-col hidden lg:flex shrink-0">
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <SidebarItem icon={FileCheck} label="Document Verification" active={activeTab === "documents"} onClick={() => setActiveTab("documents")} badge={applications.length > 0 ? applications.length : undefined} />
          <SidebarItem icon={Bell} label="Notifications" active={activeTab === "notifications"} onClick={() => setActiveTab("notifications")} />
          <SidebarItem icon={AlertTriangle} label="Complaints" active={activeTab === "complaints"} onClick={() => setActiveTab("complaints")} badge={overviewData?.complaints?.open > 0 ? overviewData.complaints.open : undefined} />
          <SidebarItem icon={Sprout} label="Crop Catalog" active={activeTab === "catalog"} onClick={() => setActiveTab("catalog")} />
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-20 bg-white px-8 flex items-center justify-between border-b border-gray-100 shrink-0">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="relative">
              ...
              {showNotificationsPopover && (
                <div className="absolute right-0 mt-2 w-[450px] bg-white border border-gray-100 rounded-2xl shadow-xl z-50 flex animate-in fade-in slide-in-from-top-2">
                  <div className="flex-1 p-4 border-r border-gray-100">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-bold text-gray-800">Notifications</h3>
                      <button onClick={handleMarkAllRead} className="text-xs font-bold text-green-600 hover:text-green-700 transition-colors">Mark all as read</button>
                    </div>
                    <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2">
                      <button onClick={() => setNotificationTab('All')} className={`text-[11px] font-bold px-3 py-1.5 rounded-full transition-colors ${notificationTab === 'All' ? 'bg-green-100 text-green-700' : 'text-gray-500 hover:bg-gray-100'}`}>All ({tabCounts.All})</button>
                      <button onClick={() => setNotificationTab('Alerts')} className={`text-[11px] font-bold px-3 py-1.5 rounded-full transition-colors ${notificationTab === 'Alerts' ? 'bg-green-100 text-green-700' : 'text-gray-500 hover:bg-gray-100'}`}>Alerts ({tabCounts.Alerts})</button>
                      <button onClick={() => setNotificationTab('Activities')} className={`text-[11px] font-bold px-3 py-1.5 rounded-full transition-colors ${notificationTab === 'Activities' ? 'bg-green-100 text-green-700' : 'text-gray-500 hover:bg-gray-100'}`}>Activities ({tabCounts.Activities})</button>
                      <button onClick={() => setNotificationTab('Transactions')} className={`text-[11px] font-bold px-3 py-1.5 rounded-full transition-colors ${notificationTab === 'Transactions' ? 'bg-green-100 text-green-700' : 'text-gray-500 hover:bg-gray-100'}`}>Transactions ({tabCounts.Transactions})</button>
                      <button onClick={() => setNotificationTab('System')} className={`text-[11px] font-bold px-3 py-1.5 rounded-full transition-colors ${notificationTab === 'System' ? 'bg-green-100 text-green-700' : 'text-gray-500 hover:bg-gray-100'}`}>System ({tabCounts.System})</button>
                    </div>
                    
                    <div className="space-y-4 max-h-[350px] overflow-y-auto pr-2">
                      {popoverActivities.filter((n: any) => notificationTab === 'All' || n.tabType === notificationTab).map((n: any) => {
                        const Icon = n.icon;
                        return (
                          <div key={n.id} className="flex gap-3 items-start">
                            <div className={`w-8 h-8 rounded-full ${n.bgClass} flex items-center justify-center shrink-0`}>
                              <Icon className={`w-4 h-4 ${n.colorClass}`} />
                            </div>
                            <div className="flex-1">
                              ...
                              <p className="text-xs font-medium text-gray-500 mt-0.5">{n.desc}</p>
                            </div>
                            {n.unread && <div className="w-1.5 h-1.5 bg-red-500 rounded-full mt-1.5"></div>}
                          </div>
                        );
                      })}
                      {popoverActivities.filter((n: any) => notificationTab === 'All' || n.tabType === notificationTab).length === 0 && (
                        <div className="text-center py-6">
                          <p className="text-xs font-semibold text-gray-400">No notifications in this tab.</p>
                        </div>
                      )}
                    </div>
                    
                    <button className="w-full mt-4 text-left text-sm font-bold text-green-600 hover:text-green-700 flex justify-between items-center">
                      View all notifications <ChevronRight className="w-4 h-4" />
                    </button>
                  </div>

                  <div className="w-[180px] p-4 bg-gray-50/50 rounded-r-2xl">
                    <p className="text-xs font-bold text-gray-800 mb-3">Notification Types</p>
                    <div className="space-y-4">
                      <div className="flex gap-2">
                        ...
                        <div>
                          <p className="text-[11px] font-bold text-gray-800">Activities</p>
                          <p className="text-[10px] text-gray-500 leading-tight mt-0.5">User actions and activities</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <div>
                          <p className="text-[11px] font-bold text-gray-800">Alerts</p>
                          <p className="text-[10px] text-gray-500 leading-tight mt-0.5">Important alerts & actions</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <div>
                          <p className="text-[11px] font-bold text-gray-800">Transactions</p>
                          <p className="text-[10px] text-gray-500 leading-tight mt-0.5">Payments and settlements</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <div>
                          <p className="text-[11px] font-bold text-gray-800">System</p>
                          <p className="text-[10px] text-gray-500 leading-tight mt-0.5">System updates & info</p>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex-1 overflow-y-auto p-8">
            ...
            {activeTab === "users-farmers" && <FarmersTab />}
            {activeTab === "users-delivery" && <DeliveryTab />}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-black text-gray-800">
                          {loadingOverview ? "..." : totalOrders6Months.toLocaleString()}
                        </span>
                        <span className={`text-[10px] font-bold flex items-center ${ordersGrowth.isUp ? 'text-green-600' : 'text-red-500'}`}>
                          {ordersGrowth.isUp ? <ArrowUp className="w-3 h-3"/> : <ArrowDown className="w-3 h-3"/>} {ordersGrowth.val}%
                        </span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-black text-gray-800">
                          {loadingOverview ? "..." : `₹${totalRevenue6Months.toLocaleString()}`}
                        </span>
                        <span className={`text-[10px] font-bold flex items-center ${revenueGrowth.isUp ? 'text-green-600' : 'text-red-500'}`}>
                          {revenueGrowth.isUp ? <ArrowUp className="w-3 h-3"/> : <ArrowDown className="w-3 h-3"/>} {revenueGrowth.val}%
                        </span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-black text-gray-800">
                          {loadingOverview ? "..." : newUsers6Months.toLocaleString()}
                        </span>
                        <span className={`text-[10px] font-bold flex items-center ${usersGrowth.isUp ? 'text-green-600' : 'text-red-500'}`}>
                          {usersGrowth.isUp ? <ArrowUp className="w-3 h-3"/> : <ArrowDown className="w-3 h-3"/>} {usersGrowth.val}%
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      <div className="p-4 border border-gray-100 rounded-2xl flex items-center justify-between bg-gray-50 hover:bg-white transition-colors group">
                        ...
                        <button 
                          ...
                          className="text-xs font-bold text-[#1B5E20] bg-green-50 hover:bg-green-100 px-4 py-2 rounded-xl transition-all flex flex-col items-center justify-center min-w-[75px]"
                        >
                          <span>Review</span>
                          <span className="text-sm mt-0.5">&rarr;</span>
                        </button>
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div onClick={() => setActiveTab("documents")} className="p-5 rounded-2xl bg-green-50 border border-green-100 hover:border-green-300 transition-colors cursor-pointer group">
                    ...
                    <div onClick={() => setActiveTab("notifications")} className="p-5 rounded-2xl bg-blue-50 border border-blue-100 hover:border-blue-300 transition-colors cursor-pointer group">
                      <div className="flex items-start justify-between mb-4">
                    ...
                    <div onClick={() => setActiveTab("complaints")} className="p-5 rounded-2xl bg-amber-50 border border-amber-100 hover:border-amber-300 transition-colors cursor-pointer group">
                      <div className="flex items-start justify-between mb-4">
                    ...
                    <div onClick={() => setActiveTab("transactions")} className="p-5 rounded-2xl bg-purple-50 border border-purple-100 hover:border-purple-300 transition-colors cursor-pointer group">
                      <div className="flex items-start justify-between mb-4">
          ...
          {/* NOTIFICATIONS TAB COMPONENT IS RENDERED AT THE TOP OF THE SWITCH */}
        </div>
      </main>
    </div>
  );
