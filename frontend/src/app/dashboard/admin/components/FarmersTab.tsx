...
import { 
  ...
  Eye, Mail, MapPin, ChevronLeft, ChevronRight, Check, X, AlertTriangle
} from 'lucide-react';
...
export default function FarmersTab() {
  ...
  const uniqueCrops = useMemo(() => {
    const allCrops = farmers
      .flatMap(f => (f.primaryCrops && f.primaryCrops !== "N/A" ? f.primaryCrops.split(',') : []))
      .map((c: string) => c.trim())
      .filter((c: string): c is string => !!c);
    return ["All Crops", ...Array.from(new Set(allCrops)).sort()];
  }, [farmers]);
  ...
  const filteredFarmers = useMemo(() => {
    return farmers.filter(farmer => {
      ...
      // Status filter
      const matchesStatus = statusFilter === "All Status" || farmer.status === statusFilter;

      // District filter: match against the city field (which holds district)
      const matchesDistrict = districtFilter === "All Districts" || farmer.city === districtFilter;

      // Crop filter: partial match in primaryCrops string
      const matchesCrop = cropFilter === "All Crops" || 
        (farmer.primaryCrops && farmer.primaryCrops !== "N/A" &&
          farmer.primaryCrops.split(',').map((c: string) => c.trim()).includes(cropFilter));

      // Joined date filter
      let matchesDate = true;
      if (dateFilter !== "All Time" && farmer.joinedOn) {
        const joinDate = new Date(farmer.joinedOn);
      ...
      return matchesSearch && matchesStatus && matchesDistrict && matchesCrop && matchesDate;
    });
  }, [farmers, searchQuery, statusFilter, districtFilter, cropFilter, dateFilter, now]);
  ...
  useEffect(() => { setCurrentPage(1); }, [searchQuery, statusFilter, districtFilter, cropFilter, dateFilter]);

  const hasActiveFilters = searchQuery || statusFilter !== "All Status" || districtFilter !== "All Districts" || cropFilter !== "All Crops" || dateFilter !== "All Time";
  ...
  const handleBlock = async () => {
    ...
    try {
      const shouldBlock = selectedFarmer.status !== "Suspended";
      await api.patch(`/admin/buyers/${selectedFarmer.id}/block`, { block: shouldBlock });
      await fetchFarmers();
      setBlockModal(false);
  ...
  const handleSendNotification = async () => {
    if (!selectedFarmer || !notifMessage.trim()) return;
    setActionLoading(true);
    try {
      // Notification endpoint integration point
      await api.post(`/notifications/send`, { userId: selectedFarmer.id, message: notifMessage });
      setNotifModal(false);
      setNotifMessage("");
      setSelectedFarmer(null);
    } catch (e) {
  ...
  const totalAllFarmers = farmers.length;
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-black text-gray-900">Farmers</h1>
        <p className="text-sm font-semibold text-gray-500 mt-1">Manage and monitor all farmer accounts.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider leading-tight">Total Farmers</span>
          </div>
          <div>
            <h3 className="text-3xl font-black text-gray-900">
              {loading ? <span className="text-gray-300">...</span> : totalAllFarmers}
            </h3>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider leading-tight">Verified Farmers</span>
          </div>
          <div>
            <h3 className="text-3xl font-black text-gray-900">
            ...
            <p className="text-xs font-semibold text-green-600 mt-1">
              {totalAllFarmers > 0 ? ((verifiedCount / totalAllFarmers) * 100).toFixed(1) : "0"}% of total
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-orange-500 mt-1">
              {totalAllFarmers > 0 ? ((pendingCount / totalAllFarmers) * 100).toFixed(1) : "0"}% of total
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-red-500 mt-1">
              {totalAllFarmers > 0 ? ((rejectedCount / totalAllFarmers) * 100).toFixed(1) : "0"}% of total
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-gray-500 mt-1">
              {totalAllFarmers > 0 ? ((suspendedCount / totalAllFarmers) * 100).toFixed(1) : "0"}% of total
            </p>
          </div>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm font-medium flex items-center justify-between">
          <span>{error}</span>
          <button onClick={fetchFarmers} className="text-red-600 font-bold hover:underline ml-4">Retry</button>
        </div>
      )}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                <th className="py-3.5 px-6 text-xs font-black text-gray-500 uppercase tracking-wider">Farmer</th>
                ...
                <tr>
                  <td colSpan={10} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm font-semibold text-gray-400">Loading farmers...</p>
                    </div>
                  </td>
                </tr>
                  ...
                  <td colSpan={10} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm font-bold text-gray-500">
                        {hasActiveFilters ? "No farmers match your filters." : "No farmers found."}
                      </p>
                      {hasActiveFilters && (
                    ...
                    <td className="py-3 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs font-medium text-gray-400 truncate">{farmer.email || "No email"}</p>
                        </div>
                      </div>
                    </td>
                    {/* Location */}
                    <td className="py-3 px-6">
                      {farmer.location && farmer.location !== "Unknown" ? (
                        <div className="flex items-center gap-1.5 text-sm font-semibold text-gray-600">
                          <MapPin className="w-3.5 h-3.5 text-gray-400 shrink-0" />
                          <span className="truncate max-w-[130px]">{farmer.location}</span>
                        </div>
                      ) : (
                    ...
                    <td className="py-3 px-6" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button
                          ...
                          onClick={() => { setSelectedFarmer(farmer); setNotifMessage(""); setNotifModal(true); }}
                          className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                        >
                          <Mail className="w-4 h-4" />
                        </button>
                        <button
                          title={farmer.status === "Suspended" ? "Unblock User" : "Block User"}
                          onClick={() => { setSelectedFarmer(farmer); setBlockModal(true); }}
                          className={`p-1.5 rounded-lg transition-colors ${farmer.status === "Suspended" ? "text-orange-500 hover:bg-orange-50" : "text-gray-400 hover:text-red-600 hover:bg-red-50"}`}
                        >
                          <Ban className="w-4 h-4" />
                        </button>
                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedRows[farmer.id] ? "rotate-180" : ""} ml-2`} />
                      </div>
                    </td>
                  </tr>
                  
                  {/* Expanded Details Row */}
                  {expandedRows[farmer.id] && (
                      ...
                      <td colSpan={5} className="p-0">
                        <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-5 gap-4">
                          <div>
                            ...
                            <p className="text-sm font-semibold text-gray-700">{farmer.phone || "N/A"}</p>
                          </div>
                          <div>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Primary Crops</p>
                            <p className="text-sm font-semibold text-gray-700 truncate" title={farmer.primaryCrops}>
                              {farmer.primaryCrops && farmer.primaryCrops !== "N/A" ? farmer.primaryCrops : "—"}
                            </p>
                          </div>
                          <div>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Listings</p>
                            <p className="text-sm font-black text-gray-900">{farmer.listings ?? 0}</p>
                          </div>
                          <div>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Total Orders</p>
                            <p className="text-sm font-black text-gray-900">{farmer.totalOrders ?? 0}</p>
                          </div>
                          <div>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Earnings</p>
                            <p className="text-sm font-black text-gray-900">₹{(farmer.earnings ?? 0).toLocaleString()}</p>
                          </div>
                        </div>
                      </td>
                    </tr>
        ...
        <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm">
          <p className="text-gray-500 font-medium">
            ...
            <span className="font-bold text-gray-900">
              {filteredFarmers.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}
            </span>
            {' '}to{' '}
            <span className="font-bold text-gray-900">
              {Math.min(currentPage * PAGE_SIZE, filteredFarmers.length)}
            </span>
            {' '}of{' '}
            <span className="font-bold text-gray-900">{filteredFarmers.length}</span>
            {' '}farmers
          </p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Sending to: <span className="font-bold text-gray-800">{selectedFarmer.name}</span>
            </p>
            <textarea
              value={notifMessage}
      ...
      {blockModal && selectedFarmer && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-black text-gray-900">
                {selectedFarmer.status === "Suspended" ? "Unblock Farmer" : "Block Farmer"}
              </h2>
              <button
                onClick={() => { setBlockModal(false); setSelectedFarmer(null); }}
                className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors"
              >
                <X className="w-5 h-5 text-gray-500" />
              </button>
            ...
            <div className="flex items-start gap-3 p-4 bg-orange-50 rounded-xl mb-4">
              ...
              <p className="text-sm font-medium text-orange-700">
                {selectedFarmer.status === "Suspended"
                  ? `Are you sure you want to unblock "${selectedFarmer.name}"? They will regain access.`
                  : `Are you sure you want to block "${selectedFarmer.name}"? They will lose platform access.`}
              </p>
            </div>
