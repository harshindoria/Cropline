    ...
    const fetchOrders = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          setOrders(allOrders.filter((o: any) => o.buyerId === user?.id));
        }
      } catch (err) {
      ...
      <header className="bg-white border-b border-gray-100 p-4 sticky top-0 z-30">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-[#1B5E20]">My Orders</h1>
          </div>
          ...
          {filteredOrders.length > 0 ? (
            filteredOrders.map(order => {
              ...
              return (
                <Link key={order.id} href={`/dashboard/buyer/orders/${order.id}`}>
                  <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group flex items-center gap-4 cursor-pointer">
                    ...
                    <div className="shrink-0 text-gray-300 group-hover:text-[#1B5E20] transition-colors pl-2 border-l border-gray-50">
            ...
            <div className="bg-white p-12 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-gray-500 font-medium max-w-sm mb-6">
                You haven't placed any orders that match the selected filter.
              </p>
              <button 
                onClick={() => router.push("/dashboard/buyer")}
